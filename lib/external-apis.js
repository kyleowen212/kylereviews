  ...
  if (type === 'tv') {
    return {
      ...
      metadata: {
        ...
        _tmdbId: String(item.id),
        _totalSeasons: item.number_of_seasons || 1,
      },
...
// ─── TMDB Season-Specific Details ───
async function getTMDBSeasonDetails(showId, seasonNumber) {
  const apiKey = process.env.TMDB_API_KEY;
  if (!apiKey) return null;

  const res = await fetch(
    `https://api.themoviedb.org/3/tv/${showId}/season/${seasonNumber}?api_key=${apiKey}&append_to_response=credits,videos,images`
  );
  const season = await res.json();

  const trailer = (season.videos?.results || []).find(
    (v) => v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser')
  );

  // Get backdrops from the show level filtered for this season, plus season poster
  const showImgRes = await fetch(
    `https://api.themoviedb.org/3/tv/${showId}/images?api_key=${apiKey}`
  );
  const showImages = await showImgRes.json();
  ...
  return {
    ...
    cast: (season.credits?.cast || []).slice(0, 5).map((a) => a.name).join(', '),
    episodeCount: season.episodes?.length || 0,
    backdrops: (showImages.backdrops || []).slice(0, 12).map((img) => ({
...
// ─── TMDB Image Browser ───
async function getTMDBImages(id, type = 'movie') {
  const apiKey = process.env.TMDB_API_KEY;
  if (!apiKey) return { backdrops: [] };

  const endpoint = type === 'tv' ? `tv/${id}` : `movie/${id}`;
  const res = await fetch(
    `https://api.themoviedb.org/3/${endpoint}/images?api_key=${apiKey}`
  );
  const data = await res.json();

  return {
    backdrops: (data.backdrops || []).slice(0, 16).map((img) => ({
      url: `https://image.tmdb.org/t/p/w1280${img.file_path}`,
      thumb: `https://image.tmdb.org/t/p/w300${img.file_path}`,
      width: img.width,
      height: img.height,
    })),
    posters: (data.posters || []).slice(0, 8).map((img) => ({
